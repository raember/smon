...
NVIDIA_GPU_COLUMNS = [
    ...
    'pcie.link.width.current', 'pcie.link.width.max', 'index', 'display_mode', 'display_active', 'persistence_mode',
    'accounting.mode', 'accounting.buffer_size', 'driver_model.current', 'driver_model.pending', 'vbios_version',
    'inforom.img', 'inforom.oem', 'inforom.ecc', 'inforom.pwr', 'gom.current', 'gom.pending', 'fan.speed [%]', 'pstate',
    'clocks_throttle_reasons.supported', 'clocks_throttle_reasons.active', 'clocks_throttle_reasons.gpu_idle',
    ...
    'clocks_throttle_reasons.sync_boost', 'memory.total [MiB]', 'memory.used [MiB]', 'memory.free [MiB]',
    'compute_mode', 'utilization.gpu [%]', 'utilization.memory [%]', 'encoder.stats.sessionCount',
    'encoder.stats.averageFps', 'encoder.stats.averageLatency', 'ecc.mode.current', 'ecc.mode.pending',
    ...
    'ecc.errors.corrected.volatile.cbu', 'ecc.errors.corrected.volatile.sram', 'ecc.errors.corrected.volatile.total',
    ...
    'ecc.errors.corrected.aggregate.cbu', 'ecc.errors.corrected.aggregate.sram', 'ecc.errors.corrected.aggregate.total',
    ...
    'retired_pages.single_bit_ecc.count', 'retired_pages.double_bit.count', 'retired_pages.pending', 'temperature.gpu',
    'temperature.memory', 'power.management', 'power.draw [W]', 'power.limit [W]', 'enforced.power.limit [W]',
    'power.default_limit [W]', 'power.min_limit [W]', 'power.max_limit [W]', 'clocks.current.graphics [MHz]',
    'clocks.current.sm [MHz]', 'clocks.current.memory [MHz]', 'clocks.current.video [MHz]',
    'clocks.applications.graphics [MHz]', 'clocks.applications.memory [MHz]',
    'clocks.default_applications.graphics [MHz]', 'clocks.default_applications.memory [MHz]',
    'clocks.max.graphics [MHz]', 'clocks.max.sm [MHz]', 'clocks.max.memory [MHz]', 'mig.mode.current',
    'mig.mode.pending'
]
...
def nvidia_smi_gpu(fields: List[str] = None, units=False) -> DataFrame:
    ...
    if len(df.columns) == 1:
        ...
        df = DataFrame(columns=NVIDIA_GPU_COLUMNS)
    return df
...
def nvidia_smi_compute(fields: List[str] = None, units=False) -> DataFrame:
    ...
    if len(df.columns) == 1:
        # "No devices were found"
        df = DataFrame(columns=NVIDIA_GPU_COMPUTE_COLUMNS)
    return df
